...
def get_sequences(query_sequences, fmt, output_dir):
    ...
    with open(query_sequences) as query_fp:
        ...
        for seq in seq_data:
            ...
            while not results_handle:
                try:
                    results_handle = NCBIWWW.qblast("blastn", "nt", seq.format(fmt), hitlist_size=10)
...
if __name__ ==  '__main__':
    ...
    parser.add_argument('query_sequences', action='store', type=str,
                        help='sequencesed or unsequencesed query sequence')
    ...
    get_sequences(args.query_sequences, args.fmt, args.output_dir)
